            ...
            <div class="flex justify-between items-center mb-8">
                <p class="text-gray-600" id="results-info">
                    ...
                    if (file_exists($searchDataPath)) {
                        $searchData = json_decode(file_get_contents($searchDataPath), true);
                        $totalAromas = count($searchData['aromas'] ?? []);
                        echo __('aromas.showing_results', 'Zeige {count} von {total} Aromen', ['count' => $totalAromas, 'total' => $totalAromas]);
                    } else {
                        echo __('aromas.showing_results', 'Zeige 0 von 0 Aromen', ['count' => '0', 'total' => '0']);
                    }
                    ?>
                </p>
                <div class="flex items-center space-x-2">
                    ...
                    <select class="border border-gray-300 rounded px-3 py-1 text-sm" id="sort-select">
                        <option value="name"><?php echo __('forms.name'); ?></option>
                        <option value="brand"><?php echo __('aromas.brand', 'Marke'); ?></option>
                        <option value="category"><?php echo __('forms.category'); ?></option>
                        <option value="rating"><?php echo __('common.popular'); ?></option>
                    </select>
                </div>
            </div>
            
            <!-- Aromas Grid -->
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" id="aromas-grid">
                
                <!-- Aromas will be populated by JavaScript -->
                <?php 
                // Load search data for server-side rendering
                $searchDataPath = __DIR__ . '/../../data/search-data.json';
                if (file_exists($searchDataPath)) {
                    $searchData = json_decode(file_get_contents($searchDataPath), true);
                    $aromas = $searchData['aromas'] ?? [];
                    
                    foreach ($aromas as $aroma): 
                ?>
                <div class="aroma-card bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow overflow-hidden" 
                     data-name="<?php echo htmlspecialchars($aroma['name']); ?>"
                     data-brand="<?php echo htmlspecialchars($aroma['brand']); ?>"
                     data-category="<?php echo htmlspecialchars($aroma['category']); ?>"
                     data-tags="<?php echo htmlspecialchars(implode(',', $aroma['tags'] ?? [])); ?>">
                    ...
                    <div class="p-4">
                        <div class="flex justify-between items-start mb-2">
                            <h3 class="font-semibold text-gray-900 text-lg"><?php echo htmlspecialchars($aroma['name']); ?></h3>
                            <button class="text-gray-400 hover:text-red-500 transition-colors">
                                <span class="material-icons text-sm">favorite_border</span>
                            </button>
                        </div>
                        
                        <p class="text-sm text-gray-600 mb-1"><?php echo htmlspecialchars($aroma['brand']); ?></p>
                        <p class="text-xs text-gray-500 mb-3"><?php echo htmlspecialchars($aroma['category']); ?></p>
                        
                        <div class="flex items-center justify-between mb-4">
                            <span class="text-sm font-medium text-vape-green"><?php echo htmlspecialchars($aroma['percentage']); ?></span>
                            <div class="flex items-center text-xs text-gray-500">
                                <span class="material-icons text-xs mr-1">star</span>
                                <span><?php echo $aroma['rating']; ?> (<?php echo $aroma['reviews']; ?>)</span>
                            </div>
                        </div>
                ...
                <?php 
                    ...
                    echo '<p class="text-center text-gray-500 col-span-full">Keine Aromen-Daten verfügbar.</p>';
                }
                ?>
